...
import { IndicadorNumerico, IndicadorPrecio, MapaPropiedad } from '@/src/components';

type SeccionPropiedadProps = {
  propiedadResponse: PropiedadDetalleResponse
}

export const SeccionPropiedad = ({ propiedadResponse }: SeccionPropiedadProps) => {
  ...
  return (
    <section id="descripcion" className="scroll-mt-28" >
      ...
      <div className="flex justify-between mb-8">
        <span className="bg-foreground text-white uppercase text-sm py-0.5 px-1.5">
          {propiedadResponse.emprendimiento?.nombre ? `${propiedadResponse.emprendimiento.nombre} - ${operacion}` : operacion}
        </span>
      </div>
      ...
      <MapaPropiedad latitud={+mapa_latitud} longitud={+mapa_longitud} className="h-80" />
